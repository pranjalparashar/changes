function printf() {
    ...
    for (var j = i + 1; j < splitWords.length; j++) {
        if (wordCount === 0) varName += splitWords[j]
        else varName += splitWords[j].replace(/^./, splitWords[j][0].toUpperCase())
        if (arrayContains(Object.keys(variables), varName)) {
            ...
            for (var k = 0; k < printQuote.length; k++) {
                if (printQuote[k] === "%" && !dictContains(identifiers, printQuote[k + 1])) {
                    printQuote = insertAtPosition(
                        printQuote,
                        k + 1,
                        identifiers[variables[varName]]
                    )
                    break
                }
            }
